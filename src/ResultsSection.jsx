...
export default function ResultsSection() {
  ...
  useEffect(() => {
    const director = () => {
      ...
      if (rp <= 0) {
        ...
        // the hand rises with the question card
        const t = ease(rp / 0.2)
        setHandTarget(mix(BELOW, SPOT, t), 0.12)
        setCard(clamp01(rp / 0.1))
      } else if (rp <= 0.28) {
        // ...and places it on the board
        setHandTarget(SPOT, 0.12)
        setCard(1 - clamp01((rp - 0.2) / 0.08))
      } else if (rp <= 0.4) {
        // then withdraws below while the audience clicks
        const t = ease((rp - 0.28) / 0.12)
        setHandTarget(mix(SPOT, BELOW, t), 0.12)
        setCard(0)
      } else {
        setHandTarget(BELOW, 0.12)
        setCard(0)
  ...
  // the whole section dissolves in place at the very end
  const out = (v) => 1 - clamp01((v - 0.93) / 0.07)
  ...
  const cardOp = useTransform(p, (v) => clamp01((v - 0.2) / 0.08) * out(v))
  const graphOutOp = useTransform(p, (v) => out(v))
